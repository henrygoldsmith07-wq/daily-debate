...
- **Full-pack, interleaved, repeated.** Arms run SERIALIZED and INTERLEAVED (`A,B,A,B,…` via `scripts/judge-study.mjs`) — never concurrently against rate-limited providers. Repeat counts meet or exceed 3/arm where provider budgets permit; per-metric raw values, mean/median/sd and bootstrap 90% CIs are published; every raw artifact is preserved under `docs/judge-runs/`.
