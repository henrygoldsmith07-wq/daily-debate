...
- The corpus campaign (1,000 debates × 3 blind ratings) is the path to human-validity claims; `/metrics` shows dashes, never placeholders, until minimum samples exist. Fixture labels are benchmark infrastructure, NOT human ground truth.

## Human-ratings immutability

Corpus ratings are **append-once**: the first submission for `(corpus_id, rater_id)` is accepted; any second submission is rejected with 409, never silently overwritten. The insert and the open→rated closure run in a single locked statement (`src/lib/corpusRatingStore.ts`), so closed items (rated / adjudicated / rejected) refuse new ratings at insert time and two simultaneous final raters serialise without losing either rating. Corrections go through the admin-only `/api/corpus/correct` path, which requires a reason and appends an immutable audit entry — previous value, new value, actor, timestamp — preserving the original record for end-to-end auditability (migration 012). Adjudication merges its consensus into the item's existing `side_mapping` instead of replacing provenance.
