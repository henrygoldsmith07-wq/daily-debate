...
- **One variable, always.** Prompt changes run as named experiments in `scripts/lib/judge-experiments.mjs`, each altering exactly one material instruction (registered arms today: `baseline` and `citation-zero-weight`, both changing only the citation clause through the `buildVerdictSystem` seam). Multi-rule prompt stacking is banned — a metric movement must map to one attributable change.
- **Full-pack, repeated.** Every arm is measured on the complete 24-fixture pack via `scripts/judge-variance.mjs` (`--runs N --experiment NAME --models …`), with per-arm dispersion (mean/median/min/max/sd) and per-gate PASS frequency aggregated from preserved raw run artifacts under `docs/judge-runs/`. No single run is treated as truth; failures are never averaged away.
