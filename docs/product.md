...
2. **Pending repair retests override that generic selector only after successful repairs.** Failed attempts remain retryable practice. Successful repairs form a durable queue: a newer success never replaces an older unresolved transfer test, and the oldest pending repair is surfaced first. Today and Progress show **Retest after repair**, the drill coach targets the same dimension, and the next solo debate on a **different topic** stores that repair's provenance in `solo_debates.coaching.repairRetest`. Replaying the same topic may be useful practice, but it is not counted as transfer. If a coaching dependency is temporarily unreadable, the debate may still start, but the categorical degradation is persisted and surfaced operationally rather than silently treated as “no coaching needed.”
