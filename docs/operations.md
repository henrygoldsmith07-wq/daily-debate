...
- **Database maintenance**: `backend-cleanup.yml` runs unattended once daily with the repository `DATABASE_URL`; it shares the `database-maintenance` concurrency group with `db-migrate.yml`, so cleanup and DDL never overlap. Production-environment reviewer gates remain reserved for explicit migrations rather than blocking scheduled housekeeping.
...
- **Production migrations** (`db-migrate.yml`): dispatch-only, never scheduled. Applies pending migrations with the repo's own ledger-based `npm run db:migrate` (idempotent via `app_migrations` - reruns only `skip`), requires the `confirm` input to be exactly `migrate`, then runs the same `--check-config` schema gate topic-generation enforces so a schema-known failure can never burn a generation retry slot. The applied/skipped migration list is mirrored into the run summary as the auditable record. `environment: production` gives admins a hook for required-reviewer approval without code changes. This is the ONLY sanctioned way production schema changes - no manual SQL.

- **Topic generation** (`topic-generation.yml`): six attempts on one retry ladder - 20:00, 21:30, 22:45, 23:40 UTC the previous evening, then 00:15 and 02:15 UTC post-midnight - all targeting the SAME tomorrow-date, because GitHub scheduled starts have been observed 4-5+ hours late and the 03:00 UTC availability SLO must survive that. The 02:15 slot doubles as the final availability verification before the deadline. Topics are WRITE-ONCE per date: the first valid write wins and later slots verify it (`already-present`) instead of regenerating. Healthy retries are READ-ONLY; the only content-replacing path is deliberate corruption repair, which runs as ONE transaction (topic update + evidence rebuild + revision-ownership invariant, COMMIT/ROLLBACK) so intermediate states like new-topic-plus-old-evidence can never be observed. The transaction is SESSION-BACKED on a dedicated connection (strategy B in `scripts/lib/sql-executor.mjs`): production's Neon HTTP `query()` calls are each their own session, so `BEGIN`/`COMMIT` issued as separate HTTP statements would silently not be a transaction - if no session can be opened the repair FAILS rather than pretending to be atomic. Normal read-only verification stays on the cheap HTTP surface. **Legacy evidence is untrusted**: evidence with a NULL topic_fingerprint has unknown revision ownership (this production served Topic A's cards under an overwritten Topic B), so healing a legacy row preserves the topic content, writes the canonical fingerprint, and REBUILDS evidence for the current topic - zero cards is a valid outcome; unknown cards are never stamped. The config gate FAILS FAST (`config-failure`) when migration 018's fingerprint schema OR migration 019's generation_reason schema/constraint is missing - no generation attempt burns a retry slot on a schema-known failure. The 15:00 UTC cycle boundary keeps post-midnight slots on the SAME date until the deadline passes, then the next evening slot advances to the next cycle. Requires the `DATABASE_URL` repository secret; `--check-config` fails fast with `config-failure` when it is absent, unreachable, or schema-incomplete. Each run records durable telemetry to `topic_run_log` (scheduledFor, actual start, scheduler delay, completedAt, duration, target date, generator outcome, generator result, provider health, content fingerprint, bounded provider attempts, freshness, result) - scheduler delay, generator failure and availability failure are separately-reported facts; a late platform start is never counted as a generator failure. The workflow artifact additionally carries the EXACT triggering slot (`cronSlot`, `scheduledFor`, `schedulerDelayMs`) so the second-witness never has to infer the slot from the nearest clock time (which understated a 125-min delay as 35 min).
- **Canonical topic persistence**: every durable `daily_topics` write in the app goes through one write-once helper (`insertCanonicalTopicOnce` in `src/lib/dailyTopic.ts`) using the query builder's `insertIgnore(values, { onConflict: "topic_date" })` surface, which generates `INSERT ... ON CONFLICT (topic_date) DO NOTHING RETURNING *` - a conflict returns ZERO rows (never an error), and the loser re-reads the winner, so concurrent request-time writers and a racing scheduled write converge on one canonical row with one fingerprint. Plain `.insert()` still fails loudly with 23505 and `upsert()` (DO UPDATE) is forbidden for this table. Full canonical shape (topic_date, title, prompt, category, sources, generation_source, generation_reason, topic_fingerprint). The overwrite-style `storePreGeneratedTopic` was removed; the request-time fallback path persists through the same helper as `request-time-fallback`. The in-memory emergency fallback stays write-free and proof-free.
- **generation_reason** (migration 019): `generation_source` says WHAT the content is (ai | fallback); `generation_reason` says HOW it was created (ai | fallback-provider-failure | fallback-policy | request-time-fallback | legacy-unknown). `legacy-unknown` is HONEST provenance: the backfill maps historical fallback rows to it instead of inventing a confident `fallback-policy`, and it is valid ONLY on rows predating 019's ledger entry (`app_migrations.applied_at`) - the freshness verifier rejects it on any newer row, so new canonical writers can never emit it. An `already-present` retry READS the stored reason and reports it unchanged - it never reconstructs or mutates it. The freshness verifier rejects unknown reasons and flags rows missing a reason on a migrated database.
...
- **Production proofs** (all six on ops health, required before "scheduling works"): database reachable, successful manual run, later successful scheduled run, same-date content idempotence (matching fingerprints from separate verified attempts), one on-time freshness-verified write before the deadline, and one real AI-generated production success. The AI proof requires ONE telemetry row carrying every fact at once (result=success + target date + the exact stored fingerprint + generatorResult=ai + freshness=true) - two rows each carrying part of it never satisfy it. Proof facts come from durable `topic_run_log` first, so `manual -> scheduled` survives the GitHub API's short run window scrolling old runs out of `per_page=8`; Actions history only reports current scheduler state. See `docs/topic-production-proof.md`.
- **Migration readiness** (schema-probed, never inferred from a count): `migration016TelemetryReady`, `migration017RouteLifecycleReady`, `migration018TopicFingerprintReady`, `migration019GenerationReasonReady` are computed from the actual required columns/constraints and exposed in ops health and the public probe; the config gate enforces 018 + 019 before any provider call.
- **Judge benchmark** (`judge-benchmark.yml`, weekly): needs at least one provider key secret. Free-tier providers enforce per-day request caps — a run landing on a depleted quota honestly reports `INSUFFICIENT DATA` / provider-reliability FAIL and is appended to `docs/judge-benchmark-attempts.json` without overwriting the last valid record. Benchmark QUALITY and artifact PUBLICATION are separate health dimensions (`benchmarkRan` / `benchmarkUsable` / `benchmarkGatesPassed` vs `artifactUploaded` / `artifactPrCreated` / `artifactMerged`): a PR-creation refusal by a repository setting is an automation failure, never evidence about judge quality, and a successful publication never implies the gates passed. The summary renders from the unit-tested truth table in `scripts/lib/benchmark-publication.mjs`.
- **Live Neon transport contract** (`neon-live.yml`, manual + weekly): optional witness that re-runs the TCP-proven guarantees (JSONB round-trip, insert-ignore, transaction commit/rollback, config gate, fingerprint identity, evidence rebuild) against a real Neon HTTP URL from the `NEON_TEST_DATABASE_URL` secret (a SCRATCH branch). Skipped green when the secret is unset; never blocks PRs.
