...
- **Second-witness delay telemetry**: `topic_run_log` stays the primary delay source, but when the production DB is unreachable - exactly when "platform late vs database down" matters most - ops health derives approximate delays from GitHub run history against the canonical ladder slots (`deriveDelayWitness`), and DB rows always win where both exist. Witness rows never claim availability facts; those stay DB-backed.
