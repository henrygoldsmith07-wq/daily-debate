...
export default async function ProgressPage() {
  ...
  return (
    <AppShell width="narrow">
      <PageHeader
        eyebrow="Skill ledger"
        title="Your trajectory"
        description={`Every metric below is recomputed deterministically from your stored argument graphs — the same pipeline that scores your debates. ${ledger.debates} completed debate${ledger.debates === 1 ? "" : "s"} analysed.`}
        actions={
          <Link href="/dna" className="btn btn-secondary px-3 py-1.5 text-xs">
            Argument DNA →
          </Link>
        }
      />
      ...
      {!enoughForClaims && (
        <p className="rounded-lg border border-[var(--rule)] bg-surface-2 px-3 py-2 text-xs text-ink3">
