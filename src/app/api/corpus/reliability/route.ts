...
// dispersion, confidence, unresolved disagreements, ground-truth gate) are
// computed by the SINGLE implementation in corpusMetrics — this route adds
// only what lives nowhere else: ICC, the adjudication queue, presentation
// balance, and strata coverage.
...
export async function GET() {
  ...
  // Shared stats from the single corpus-metrics implementation (κ, consensus
  // readiness, dispersion, confidence, ground-truth gate) — no second math.
  const metrics = computeCorpusMetrics(
    (items ?? []) as MetricItem[],
    ratings.map((r): MetricRating => ({
  ...
  const presentationBalance = {
    aFirst: ratings.filter((r) => (r.presented_first ?? "a") === "a").length,
