...
function report(over: Partial<OpsHealthReport> = {}): OpsHealthReport {
  return {
    ...
    judge: {},
    database: { status: "healthy", reachable: true, latencyMs: 5, migrationsApplied: 14, requiredTablesOk: true, missingTables: [], note: null, migrationReadiness: { migration016TelemetryReady: true, migration017RouteLifecycleReady: true, migration018TopicFingerprintReady: true, migration019GenerationReasonReady: true, note: null } },
    app: {},
  ...
  it("tracks failure states honestly instead of green-washing", () => {
    const failed = derivePublicHealthState(
      report({
        ...
        database: { status: "healthy", reachable: true, latencyMs: 5, migrationsApplied: 14, requiredTablesOk: true, missingTables: [], note: null, migrationReadiness: { migration016TelemetryReady: true, migration017RouteLifecycleReady: true, migration018TopicFingerprintReady: false, migration019GenerationReasonReady: false, note: null } },
      } as unknown as OpsHealthReport),
      now,
    );
