...
export async function discoverCandidateUrls(
  ...
  const timeoutMs = Math.max(1, opts.timeoutMs ?? 8_000);
...
/**
 * Convert the configured end-to-end budget into phase-local timeouts.
 * Discovery and retrieval are sequential phases, so retrieval must inherit
 * only the time discovery left behind rather than getting a fresh 9 seconds.
 */
